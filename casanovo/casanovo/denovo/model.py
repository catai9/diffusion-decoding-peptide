...
from ..denovo.decoderDS import DiffusionDecoderDS
from ..denovo.decoderDM1 import DiffusionDecoderDM1
from ..denovo.decoderDM2 import DiffusionDecoderDM2
...
class Spec2Pep(pl.LightningModule, ModelMixin):
    ...
    calculate_precision : bool
        ...
        This is expensive.
    decoder_type : str
        Which type of decoder to use (transformer, DS, DM1, DM2)
        This is new.
    use_knapsack_search : bool
        Whether to use knapsack search during decoding.
        This is new.
    loss_fcn : str
        Which type of loss function to use (ce, we, dinoiser)
        This is new.
    **kwargs : Dict
    ...
    def __init__(
        ...
        use_knapsack_search: bool = False,
        loss_fcn: str = "ce",
        **kwargs: Dict,
    ):
        super().__init__()
        ...
        if decoder_type == "DS":
            self.decoder = DiffusionDecoderDS(
                dim_model=dim_model,
                n_head=n_head,
                dim_feedforward=dim_feedforward,
                n_layers=n_layers,
        ...
        elif decoder_type == "DM1":
            self.decoder = DiffusionDecoderDM1(
                dim_model=dim_model,
        ...
        elif decoder_type == "DM2":
            self.decoder = DiffusionDecoderDM2(
                dim_model=dim_model,
                max_length=max_length,
                residues=residues,
                max_charge=max_charge,
        ...
        self.use_knapsack_search = use_knapsack_search
    ...
    def forward(
        ...
        """
        if self.use_knapsack_search:
            return self.knapsack_search_decode(
    ...
    def knapsack_search_decode(
        self, spectra: torch.Tensor, precursors: torch.Tensor
    ) -> List[List[Tuple[float, np.ndarray, str]]]:
        """
        Knapsack search decoding of the spectrum predictions.
        ...
        pred_peptides : List[List[Tuple[float, np.ndarray, str]]]
            ...
            peptide prediction consists of a tuple with the peptide score,
            the amino acid scores, and the predicted peptide sequence.
        """
        ...
        candidates = [[] for _ in range(batch)]  # List of (score, tokens, aa_scores) per spectrum
        completed = collections.OrderedDict((i, []) for i in range(batch))

        # Get initial predictions
        pred, _ = self.decoder(None, precursors, memories, mem_masks)
        initial_scores = pred[:, 0, :]  # Shape: (B, V)

        # Start with single amino acid predictions
        for b in range(batch):
            top_scores, top_tokens = torch.topk(initial_scores[b], min(vocab, 5))  # Limit initial candidates
            for score, token in zip(top_scores, top_tokens):
                tokens = torch.tensor([token.item()], device=self.encoder.device)
                aa_scores = torch.tensor([score.item()], device=self.encoder.device)
                candidates[b].append((score.item(), tokens, aa_scores))

        # Knapsack-style decoding loop
        for step in range(1, self.max_length):
            new_candidates = [[] for _ in range(batch)]
            for b in range(batch):
                if not candidates[b]:
                    continue

                # Expand each candidate
                batch_tokens = torch.stack([c[1] for c in candidates[b]])
                batch_precursors = einops.repeat(precursors[b:b+1], "B L -> (B S) L", S=len(candidates[b]))
                batch_memories = einops.repeat(memories[b:b+1], "B L V -> (B S) L V", S=len(candidates[b]))
                batch_masks = einops.repeat(mem_masks[b:b+1], "B L -> (B S) L", S=len(candidates[b]))

                pred, _ = self.decoder(batch_tokens, batch_precursors, batch_memories, batch_masks)
                next_scores = pred[:, step, :]  # Shape: (num_candidates, V)

                for i, (prev_score, prev_tokens, prev_aa_scores) in enumerate(candidates[b]):
                    top_scores, top_tokens = torch.topk(next_scores[i], min(vocab, 3))  # Limit branching
                    for score, token in zip(top_scores, top_tokens):
                        new_tokens = torch.cat([prev_tokens, token.unsqueeze(0)])
                        new_aa_scores = torch.cat([prev_aa_scores, score.unsqueeze(0)])
                        new_score = prev_score + score.item()

                        # Check if finished or needs to be cached
                        finished, fits_precursor, discarded = self._finish_beams(
                            new_tokens.unsqueeze(0), precursors[b:b+1], step
                        )
                        if finished[0] and not discarded[0]:
                            peptide = self.decoder.detokenize(new_tokens)
                            if fits_precursor[0]:
                                completed[b].append((new_score, new_aa_scores.cpu().numpy(), peptide))
                        elif not discarded[0]:
                            new_candidates[b].append((new_score, new_tokens, new_aa_scores))

            # Prune candidates using knapsack logic
            candidates = []
            for b in range(batch):
                if not new_candidates[b]:
                    candidates.append([])
                    continue
                # Sort by score and filter by mass constraint
                sorted_candidates = sorted(new_candidates[b], key=lambda x: x[0], reverse=True)
                pruned = []
                precursor_mz = precursors[b, 2].item()
                precursor_charge = precursors[b, 1].item()
                for score, tokens, aa_scores in sorted_candidates[:10]:  # Limit to top 10
                    peptide = self.decoder.detokenize(tokens)
                    try:
                        calc_mz = self.peptide_mass_calculator.mass(seq=peptide, charge=precursor_charge)
                        delta_mass = abs(_calc_mass_error(calc_mz, precursor_mz, precursor_charge, 0))
                        if delta_mass < self.precursor_mass_tol or calc_mz < precursor_mz:  # Allow growth
                            pruned.append((score, tokens, aa_scores))
                    except KeyError:
                        continue
                candidates.append(pruned[:5])  # Keep top 5 viable candidates

        # Finalize predictions
        pred_peptides = []
        for b in range(batch):
            if completed[b]:
                top_pred = max(completed[b], key=lambda x: x[0])  # Best scoring peptide
                pred_peptides.append([top_pred])
            else:
                pred_peptides.append([])  # No valid peptide found
    ...
    def _cache_finished_sequences(
        self,
        tokens: torch.Tensor,
        scores: torch.Tensor,
        step: int,
        finished_sequences: torch.Tensor,
        sequence_fits_precursor: torch.Tensor,
        pred_cache: collections.OrderedDict
    ):
        """
        Cache the finished sequences, storing the peptide score, tokens (amino acids),
        and whether the sequence fits the precursor m/z tolerance.

        Parameters
        ----------
        tokens : torch.Tensor of shape (n_spectra * n_beams, max_length)
            The tokens (amino acid predictions) generated at each decoding step.
        scores : torch.Tensor of shape (n_spectra * n_beams, max_length, n_amino_acids)
            The scores associated with the tokens.
        step : int
            The current decoding step.
        finished_sequences : torch.Tensor of shape (n_spectra * n_beams)
            Boolean tensor indicating which sequences have finished decoding.
        sequence_fits_precursor : torch.Tensor of shape (n_spectra * n_beams)
            Boolean tensor indicating if the sequence fits the precursor m/z tolerance.
        pred_cache : collections.OrderedDict
            The cache for storing finished peptide sequences.
        """
        # Iterate over each sequence (spectrum)
        for b in range(tokens.shape[0]):
            if finished_sequences[b]:  # Process only the finished sequences
                # Extract the sequence up to the current step
                peptide_tokens = tokens[b, : step + 1]
                peptide_score = scores[b, step].item()  # Assuming you take the score of the last token

                # Detokenize the peptide (convert token sequence back to a string of amino acids)
                peptide_sequence = self.decoder.detokenize(peptide_tokens.tolist())

                # Check if the peptide fits the precursor m/z tolerance
                m_z_fits = sequence_fits_precursor[b]

                # Add the finished sequence to the cache
                pred_cache[b].append(
                    (peptide_score, peptide_tokens.cpu().numpy(), peptide_sequence, m_z_fits)
                )
    ...
    def training_step(
        ...
        *args,
        ...
        """
        if self.decoder_type == "DM1" or self.decoder_type == "DM2":
            print(f"Custom loss function for {self.decoder_type}")
            if self.loss_fcn == "we":
                print(f"Loss Function type: {self.loss_fcn}")
                pred, truth = self.decoder.training_step(*batch, self.encoder)
                ce_loss = F.cross_entropy(pred.view(-1, pred.size(-1)), truth.view(-1))
            ...
            elif self.loss_fcn == "dinoiser":
                print(f"Loss Function type: {self.loss_fcn}")
                # Copied from https://github.com/yegcjs/DINOISER/tree/main
                pred, truth = self.decoder.training_step(*batch, self.encoder)
                noise_scale = 0.5

                # Flatten logits and target for easier computation
                logits = pred.view(-1, pred.size(-1))
                target = truth.view(-1)
                ...
                # Apply noise scale clipping during training
                noise = torch.randn_like(masked_logits) * noise_scale
                masked_logits = masked_logits + noise

                # Compute log probabilities
                log_probs = F.log_softmax(masked_logits, dim=-1)

                # Cross-entropy loss with manipulated logits
                loss = F.nll_loss(log_probs, masked_target, reduction='mean')
            elif self.loss_fcn == "ce":
                print(f"Loss Function type: {self.loss_fcn}")
                pred, truth = self.decoder.training_step(*batch, self.encoder)
                if mode == "train":
                    loss = F.cross_entropy(pred.view(-1, pred.size(-1)), truth.view(-1), label_smoothing=0.01)
                else:
                    loss = F.cross_entropy(pred.view(-1, pred.size(-1)), truth.view(-1))
        else: 
            print(f"Computing default loss function: {self.loss_fcn}")
