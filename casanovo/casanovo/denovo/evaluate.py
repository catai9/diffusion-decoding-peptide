...
def aa_match_prefix(
    ...
    """
    aa_matches = np.zeros(max(safe_len(peptide1), safe_len(peptide2)), np.bool_)
    # Find longest mass-matching prefix.
    i1, i2, cum_mass1, cum_mass2 = 0, 0, 0.0, 0.0
    while i1 < safe_len(peptide1) and i2 < safe_len(peptide2):
...
def aa_match_prefix_suffix(
    ...
    i1, i2 = safe_len(peptide1) - 1, safe_len(peptide2) - 1
...
def aa_match_batch(
    ...
    for peptide1, peptide2 in zip(peptides1, peptides2):
        ...
        n_aa1, n_aa2 = n_aa1 + safe_len(peptide1), n_aa2 + safe_len(peptide2)
...
def aa_match_metrics(
    ...
    pep_precision = sum([aa_matches[1] for aa_matches in aa_matches_batch]) / (
        safe_len(aa_matches_batch) + 1e-8
    )
