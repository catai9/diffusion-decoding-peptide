...
class ModelRunner:
    ...
    def __init__(
        ...
        overwrite_ckpt_check: Optional[bool] = False,
    ) -> None:
        """Initialize a ModelRunner"""
        self.config = config
        self.model_filename = model_filename
        self.output_dir = Path(self.config.model_save_folder_path)
        output_dir = Path(self.config.model_save_folder_path)
    ...
    def log_metrics(self, test_index: AnnotatedSpectrumIndex) -> None:
        ...
        with test_index as t_ind:
            for true_idx in range(t_ind.n_spectra):
                ...
                data = self.writer.psms[pred_idx]

                # Extract the tuple containing the 'index'
                nested_tuple = data[1]

                # Get the second element from the nested tuple which contains the 'index' string
                index_str = nested_tuple[1]

                # Split the string at '=' and convert the result to an integer
                index_value = int(index_str.split('=')[1])

                print(f"index_value = {index_value}")
                print(f"data[0] = {data[0]}")

                actual_idx_str = t_ind.get_spectrum_id(true_idx)
                actual_idx = actual_idx_str[1].split('=')[1]

                print(f"actual_idx = {actual_idx}")

                if pred_idx < len(self.writer.psms):
                    #  and index_value + 1 == actual_idx
                    seq_pred.append(data[0])
                    pred_conf.append(float(data[2]))
                    pred_idx += 1
                else:
    ...
    def initialize_model(self, train: bool) -> None:
        ...
        model_params = dict(
            ...
            use_knapsack_search=self.config.use_knapsack_search,
            loss_fcn=self.config.loss_fcn,
        )
